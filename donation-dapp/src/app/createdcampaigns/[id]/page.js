...
export default function DetailsPage () {
    const { id } = useParams();
    // State for the specific campaign details
    ...
    // When campaigns data is available, filter out the campaign with the matching id
    useEffect(() => {
        if (campaigns && Array.isArray(campaigns)) {
            console.log("Campaigns:", campaigns);
            // Adjust the property used for matching if your campaign object structure differs.
            const selectedCampaign = campaigns.find(
                (c) => c.id?.toString() === id // assuming the campaign object has an 'id' field
            );
