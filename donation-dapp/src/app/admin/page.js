...
export default function AdminPage() {
  ...
  useEffect(() => {
    const combined = [...(campaign || []), ...(deletedCampaign || [])];
    ...
    async function fetchAllDonations() {
      ...
      try {
        ...
        const allDonations = await Promise.all(
          ...
          donations.filter((donation) => donation !== null)
  ...
  const filteredCampaigns = campaigns.filter((campaign) => {
    ...
    if (selectedTab === "funded") {
      return (
        campaign.fundsReleased === true && !campaign.isDeleted && matchesSearch
      );
    }
    return false;
  });
  ...
  const approvedCampaigns =
    campaign?.filter((c) => c.approved && !c.fundsReleased).length || 0;
  ...
  return (
    <>
      <div className="min-h-screen bg-black pb-10">
        ...
        <h1 className="text-3xl mt-16 font-black text-center flex md:text-left text-white mx-30">
          Admin Dashboard
        </h1>

        <p className="mx-10 md:mx-30 mt-8 text-white flex flex-col">
          <span className="text-[18px] text-[#747474] font-semibold">
            Contract Balance
          </span>
          <span className="font-bold text-2xl mt-2">
            {contractBalance
              ? `${parseFloat(
                  ethers.formatEther(contractBalance.value)
                ).toFixed(3)} ETH`
              : "Loading..."}
          </span>
        </p>
        ...
        <div className="mt-7 grid grid-cols-1 space-y-8 md:space-y-0  md:grid-cols-3 spaxe-x-0 md:space-x-6 px-10 md:px-30 ">
          <div className="bg-[var(--dark-gray)] flex flex-col px-6 py-8 space-y-4 border-l-4 border-[var(--bold-blue)]">
            <h1 className="font-semibold text-[18px] text-[#747474]">
              Pending Approvals
            </h1>
        ...
        <div className="bg-black md:bg-[var(--dark-gray)] w-full md:w-[85%] pt-14 md:mt-12 flex flex-col mx-auto rounded-xl mb-20">
          ...
          <div
            id="editModal"
            className="overflow-x-auto overflow-y-auto mt-10 mb-4"
          >
            <div className="min-w-[800px]">
              ...
              {loading ? (
                <p className="text-[#747474] text-center font-semibold py-6">
                  Loading donations...
                </p>
                ...
                paginatedDonations.map((donation, index) => (
                  <div
                    key={index}
                    className="flex flex-row justify-between text-sm border-b border-[#1E1E1E] last-of-type:border-b-0 text-white px-10 lg:px-14 py-6 font-bold whitespace-nowrap"
                  >
                    <div className="min-w-[180px] text-left text-[#747474]">
                      {`${donation.donor.slice(0, 10)}...${donation.donor.slice(
                        -4
                      )}`}
                    </div>
                    ...
                    <div className="min-w-[180px] text-left">
                      ...
                      >
                        {donation.txHash.slice(0, 15)}...
                        {donation.txHash.slice(-5)}
                      </Link>
                    </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 lg:grid-cols-2 gap-10 mt-8 px-10 pb-10">
          ...
          {filteredCampaigns.map((campaign) => (
            <div
              key={`${campaign.name}-${campaign.targetDate}`}
              className="bg-[var(--dark-gray)] rounded-xl "
            >
              {/* Example Card */}
              <div className="">
                {campaign.imageUrl ? (
                  <img
                    ...
                    className="object-cover rounded-t-xl w-full h-52 mb-0"
                  />
                ) : (
                  ...
                  {campaign.fundsReleased ? (
                    ...
                    <>
                      <div>
                        ...
                        <p className="text-[#747474] text-[14px]">
                          Raised of {ethers.formatEther(campaign.targetAmount)}{" "}
                          ETH
                        </p>
                      </div>
                      <div className="flex flex-col items-center">
                ...
                <div className="flex flex-row justify-between items-center">
                    ...
                    <div>
                      <p className="text-[#747474] text-[14px]">
                        ...
                        <span className="font-extrabold text-[14px]">
                          {" "}
                          {truncateAddress(campaign.organization)}
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  {campaign.isDeleted ? (
                    ...
                    <Link href={`/donate/${campaign.id}`}>
                      <button className="text-[var(--sblue)] ml-3 md:mx-0 mt-2 text-[14px] whitespace-nowrap flex cursor-pointer">
                        See <span className="hidden md:block mx-1">full</span>{" "}
                        details
                      </button>
                    </Link>
                  )}
