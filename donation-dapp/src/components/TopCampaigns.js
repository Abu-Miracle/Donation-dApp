...
export default function TopCampaigns() {
  ...
  const topFour = useMemo(() => {
    ...
    return [...allCampaigns]
      .filter(
        (c) => c.approved && !c.isDeleted && Number(c.targetDate) > nowSec
      )
      .sort(
        (a, b) =>
  ...
  return (
    <div className="md:min-h-[70vh] bg-[var(--bold-blue)] py-10 relative">
      <h1 className="font-black text-white text-3xl mb-6 md:mb-8 px-10">
        Our Top Campaigns
      </h1>
      ...
      {!isConnected && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-10">
          <p className="text-white text-lg font-semibold text-center">
            Please connect an Ethereum wallet to view our top campaigns
          </p>
        </div>
      )}
      ...
      <div>
        ...
        {isConnected ? (
          <div className="flex justify-end">
            <Link
              href="/donate"
              className="mt-4 text-white font-medium underline underline-offset-2 font-sans px-10"
            >
              Browse More
            </Link>
          </div>
